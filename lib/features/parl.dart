...
class FileDownloadScreenOptimal extends StatefulWidget {
  @override
  _FileDownloadScreenOptimalState createState() => _FileDownloadScreenOptimalState();
}

class _FileDownloadScreenOptimalState extends State<FileDownloadScreenOptimal> {
  double progress = 0.0;
  String progressText = '0 MB / 0 MB'; // To display the file size
  CancelToken cancelToken = CancelToken(); // Create a CancelToken

  // Function to start downloading the file with optimized settings
  Future<void> downloadFile(String url, String fileName) async {
    Dio dio = Dio(
      BaseOptions(
        receiveTimeout: Duration(minutes: 1),  // Increase timeout to prevent disconnections
        connectTimeout: Duration(seconds: 30), // Increase connection timeout
        responseType: ResponseType.bytes,      // Download data as bytes
        followRedirects: false,                // Disable unnecessary redirects
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Optimized File Download')),
      body: Padding(
